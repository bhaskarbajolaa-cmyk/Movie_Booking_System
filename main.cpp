...
class CinemaMenu {
 private:
  ...
  Payment* processPaymentInput() {
    ...
    if (pChoice == 2) return new CardPayment("6934-2314-1234-4321");
  ...
  Show* selectShow() {
    ...
    for (size_t i = 0; i < availableShows.size(); ++i) {
      cout << "  [" << i + 1 << "] Screen-"
           << availableShows[i]->getScreen()->getScreenNo() << " "
           << availableShows[i]->getStartTime() << "\n";
    }
    cout << "Choose show: ";
  ...
  void printReceipt(const vector<ShowSeat*>& selectedSeats, float total) {
    ...
    for (ShowSeat* ss : selectedSeats) {
      Seat* s = ss->getSeat();
      char prefix = (s->getSeatType() == "SILVER")
                        ? 'A'
                        : (s->getSeatType() == "GOLD" ? 'B' : 'C');
      cout << "  " << prefix << s->getSeatNo() << " " << s->getSeatType()
           << " Rs." << priceCalculator->calculateSeatPrice(s) << "\n";
    }
    cout << "  TOTAL          Rs." << total << "\n";
  }
  ...
  void handleBooking() {
    ...
    Booking* b = bookingService->createBooking(currentCustomer, selectedShow,
                                               selectedSeats, total);
    if (b) {
  ...
  void handleCancellation() {
    if (myBooking && myBooking->getStatus() == "CONFIRMED") {
      if (bookingService->cancelBooking(myBooking->getBookingId())) {
        cout << "Booking " << myBooking->getBookingId()
 ...
 public:
  CinemaMenu(Cinema* c, BookingService* bs, PriceCalculator* pc,
             TicketPrinter* tp)
  ...
  ~CinemaMenu() { delete currentCustomer; }
